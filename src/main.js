...
async function main() {
  ...
  if (config.homeAssistant.baseUrl && config.homeAssistant.token) {
    ...
    homeAssistant = createSmartHomeAssistantClient({
      ...
      logger,
      // Mismo criterio que el temperature-watcher (LUI-TSK-0081): la media que
      // se responde al preguntar y la que dispara el aviso deben coincidir.
  ...
  if (config.dishwasher.enabled && homeAssistantStateCache) {
    ...
    dishwasherWatcher = createDishwasherWatcher({
      ...
      idleIntervalMs: config.dishwasher.idleIntervalMs,
      activeIntervalMs: config.dishwasher.activeIntervalMs,
