...
export function createWebApp(deps) {
  const { llmService, statusService, rulesRepository, urlFetcher, webSearch, homeAssistant, notificationService, prometheusClient, watchtowerWebhookToken, aptHealthWebhookToken, logger, host, port } = deps;
  ...
  registerWebRoutes({ registry, llmService, statusService, rulesRepository, urlFetcher, webSearch, homeAssistant, notificationService, prometheusClient, watchtowerWebhookToken, aptHealthWebhookToken, logger });
